...
public class ParserFunction extends OpVisitorBase {
...
//            withoutWhere = withoutWhere.with(WithList.toArray(new AliasedExpression[0]));
        ...
        Object object = varMap.getVarMapDSL().get(ProjectVariable.getFirst());
      //  boolean isNamedInstance = object instanceof Named;
       // System.out.println("Is the object an instance of Named? " + isNamedInstance);
        if (object instanceof Named)
            withReturn = buildReturn(withOptionalWhere,RebuildProject.rebuildNamedProject(ProjectVariable).toArray(new Named[0]));
        else
            withReturn = buildReturn(withOptionalWhere,RebuildProject.rebuildExpressionProject(ProjectVariable, ExtendProject).toArray(new Expression[0]));
    ...
    private void buildPendingRelationships(Triple triple) {
        ...
        if (!predicate.isConcrete()) {
            Relationship rel = Cypher.anyNode(subjectName).relationshipTo(Cypher.anyNode(object.getName()), predicateURI)
                    .named(predicateName);
            ParseTriples.putVarToMap(predicate.getName(), Cypher.type(rel));
        }
        if (!optionalFlag)
        ...
        switch (BuildUtils.getPredicateType(predicate)) {
            case "Type" :
                if (object.isConcrete()) {
                    ...
                    ParseTriples.putVarToMap(object.getName(), Cypher.labels(Cypher.anyNode(subjectName)));
                break;
            case "Property" :
                if (object.isConcrete()) {
                    if (object.isLiteral())
